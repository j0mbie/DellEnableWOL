...
#>
	...
	If (Get-Item -Path ("DellSmBios:\" + $BIOSItem) -ErrorAction SilentlyContinue) {
		Write-Host ("BIOS" + [char]32 + $BIOSItem.split('\')[0] + "\" + $BIOSItem.split('\')[1] + ":" + [char]32) -NoNewline
		...
		Write-Host ("BIOS" + [char]32 + $BIOSItem.split('\')[0] + "\" + $BIOSItem.split('\')[1] + ":" + [char]32) -NoNewline
		Write-Host "Doesn't exist. Ignoring." -ForegroundColor Yellow
	}
	
	#Disable CState Control
	$BIOSItem = "Performance\CStatesCtrl"
	$NewValue = "Disabled"
	#Test if Performance\CStateCtrl exists
	If (Get-Item -Path ("DellSmBios:\" + $BIOSItem) -ErrorAction SilentlyContinue) {
		Write-Host ("BIOS" + [char]32 + $BIOSItem.split('\')[0] + "\" + $BIOSItem.split('\')[1] + ":" + [char]32) -NoNewline
		If ((Get-Item -Path ("DellSmBios:\" + $BIOSItem)).CurrentValue -ne $NewValue) {
			If ($BIOSPassword) {
	...
	$NewValue = "Disabled"
	#Test if PowerManagement\CStatesCtrl exists
	If (Get-Item -Path ("DellSmBios:\" + $BIOSItem) -ErrorAction SilentlyContinue) {
		Write-Host ("BIOS" + [char]32 + $BIOSItem.split('\')[0] + "\" + $BIOSItem.split('\')[1] + ":" + [char]32) -NoNewline
		If ((Get-Item -Path ("DellSmBios:\" + $BIOSItem)).CurrentValue -ne $NewValue) {
			If ($BIOSPassword) {
...
Function Set-AdvancedNIC {
	...
	$NICS = Get-NetAdapter -Physical | Where-Object {($_.PhysicalMediaType -eq '802.3')}
	ForEach($NIC in $NICS) {
...
#>
	...
	#Turn off Fast Startup (HiberBoot)
    Write-Host ("OS Fast Startup (HiberBoot):" + [char]32) -NoNewline
    ...
    $PNPValue = 256
	$Adapter = Get-NetAdapter -Physical | Where-Object { ($_.PhysicalMediaType -eq '802.3') }
